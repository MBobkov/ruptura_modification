...
Purge::Purge(std::string _displayName, std::vector<Component> _components, size_t _carrierGasComponent,
...
void Purge::initialize()
{
  ...
  for (size_t i = 0; i < Ngrid + 1; ++i)
  {
    ...
    for (size_t j = 0; j < Ncomp; ++j)
    {
      ...
      sum1 += Yi1[j];
      //std::cout << i << " " << Yi1[j] << " " << sum1 << " " << P[i * Ncomp + j] << " " << Pt[i] << std::endl;
    }
    for (size_t j = 0; j < Ncomp; ++j)
    {
      Yi1[j] /= sum1;
    ...
    for (size_t j = 0; j < Ncomp; ++j)
    {
      ...
      //std::cout << Q[i * Ncomp + j] << " " << i << " " << Qeq1[i * Ncomp + j] << " " << P[i * Ncomp + j] <<  " " << Yi1[j] << std::endl;
    }
  }
...
void Purge::computeFirstDerivatives(std::vector<double> &dqdt, std::vector<double> &dpdt, std::vector<double> &dTdt, std::vector<double> &dTdtWall,
...
void Purge::computeVelocityTemperatureLight()
{
  ...
  if (i > indexRight) {
    ...
    Vnew[i] = Vnew[i - 1] + dx * (sum - Vnew[i - 1] * dPdz) / Pt[i] + thermal_expansion + dx * (DTdtnew[i] / Tgsnew[i]);
    //std::cout << sum * 0.01 * dx / Pt[i] << " " << i << std::endl;
    //std::cout << dx * (sum - Vnew[i - 1] * dPdz) / Pt[i] << " " << i << std::endl;
  }
  ...
  // explicit version
  Vnew[Ngrid] = Vnew[Ngrid - 1] + dx * (sum - Vnew[Ngrid - 1] * dPdz) / Pt[Ngrid];// + dx * (DTdtnew[Ngrid] / Tgsnew[Ngrid]);


}

void Purge::computeVelocityTemperature()
{
  double idx2 = 1.0 / (dx * dx);
